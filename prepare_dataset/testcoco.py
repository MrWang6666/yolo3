...
for catName in CATS_NAME:
    ...
    i = 0
    ann_line = ''
    preparedAnnFile = os.path.join(COCO_ANN_PATH, catName + ".txt")
    ...
    for i, imgId in enumerate(imgIds):
        ...
        ann_filename = img[0]['file_name']
        if ann_filename == "000000458755.jpg":
            img_file = pimg.open("{}/{}".format(COCO_ANN_IMG, ann_filename))
            imgdraw = draw.ImageDraw(img_file)

            # 获得该张图的类型和bbox的标签
            for j, ann in enumerate(anns):

                category_id, bbox = ann['category_id'], ann['bbox']
                print("category_id, bbox: ", category_id, bbox)

                print("{}/{}".format(COCO_ANN_IMG, ann_filename))

                box = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
                center = (bbox[0] + bbox[2] / 2, bbox[1] + bbox[3] / 2, bbox[0] + bbox[2] / 2 + 2, bbox[1] + bbox[3] / 2 + 2)

                imgdraw.rectangle(box, outline='red')
                imgdraw.rectangle(center, fill='red')
